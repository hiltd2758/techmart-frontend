    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      if (refreshToken) {
        try {
          ...
          console.error("[Customer API] Làm mới token thất bại:" + err.message);
          localStorage.clear();
...
export const customerAPI = {
   ...
   */
  getCustomerById: (id) => api.get(`/customers/profile/${id}`),
   ...
   */
  updateCustomer: (id, data) => api.put(`/customers/profile/${id}`, data),
   ...
   */
  deleteCustomer: (id) => api.delete(`/customers/profile/${id}`),
   ...
   */
  getAddresses: () => api.get(`/public/customer/addresses`),

  /**
   * Lấy địa chỉ mặc định (công khai)
   * @returns {Promise} Response chứa địa chỉ mặc định
   */
  getDefaultAddress: () => api.get(`/public/customer/addresses/default`),
   ...
   */
  createAddress: (data) => api.post(`/public/customer/addresses`, data),
   ...
   * @returns {Promise} Response xác nhận xóa
   */
  deleteAddress: (id) => api.delete(`/public/customer/addresses/${id}`),
};
...
export const authAPI = {
   ...
   * @returns {Promise} Response từ server
   */
  verifyUser: (token) => api.post(`/auth/verification/${token}`),
